...
def mAP(pre_bboxes, GT, num_cls, iou_thresh=0.5, use_07_metric=True, e=0.01):
    ...
    pre_bboxes=copy.deepcopy(pre_bboxes)
    GT=copy.deepcopy(GT)
    n_GT = {}
    for name in pre_bboxes.keys():
        bboxes = pre_bboxes[name]
...
if __name__ == "__main__":
    ...
    pre_bboxes = joblib.load('/home/yuqing/PycharmProjects/Demo35/SSD_mobilenet_v1/test/SSD300_2x.pkl')
    # pre_bboxes=joblib.load('/home/zhai/PycharmProjects/Demo35/pytorch_Faster_tool/test/py_Faster_vgg16_1.pkl')
    AP = mAP(pre_bboxes , GT, 20, iou_thresh=0.5, use_07_metric=True, e=0.01)
    AP=AP[~np.isnan(AP)]
    AP = AP.mean()
    print(AP)
